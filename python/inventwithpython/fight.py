...
# Lower thac0 and lower ac values are always better. Higher damage and hp values are better.
...
def attack(attacker, defender):
    if random.randint(1, 20) >= attacker["thac0"] - defender["ac"]:
        ...
        display("%s (%s hp) hits %s (%s hp) for %s points of damage. %s is reduced to %s hp"
            % (attacker["name"], attacker["hp"], defender["name"], defender["hp"], damage,
            defender["name"], defender["hp"] - damage))
        defender["hp"] -= damage
...
print("Alice won %s (%s%%) fights. Bob won %s (%s%%) fights."
    % (aliceWins, round(float(aliceWins) / NUM_FIGHTS * 100, 2),
       bobWins, round(float(bobWins) / NUM_FIGHTS * 100, 2)))
