...
public class Creature {
    ...
    public Creature(World world, String name, char glyph, Color color, int maxHp, int attack,
                    int defense) {
        this.world = world;
        this.name = name;
        this.glyph = glyph;
    ...
    public void moveBy(int mx, int my, int mz) {
        ...
        if (mz == -1) {
            ...
            if (tile == Tile.STAIRS_UP)
                ...
                doAction("try to down but are stopped by the cave floor");
                return;
            }
        }
    ...
    private void throwAttack(Item item, Creature other) {
        commonAttack(other, attackValue / 2 + item.thrownAttackValue(),
                     "fire a %s at the %s for %d damage", item.name(), other.name);
    }
    ...
    public void doAction(String message, Object ... params) {
        ...
        for (int oy = -r; oy < r + 1; oy++) {
            ...
            if (other == this)
                ...
                other.notify(String.format("The %s %s.", name, makeSecondPerson(message)),
                             params);
        }
    }
