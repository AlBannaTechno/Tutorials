...
fn main() {
    ...
    let rectangle: Rectangle = Rectangle { p1: origin(), p2: Point { x: 3.0, y: 4.0 }};

    // Heap allocated rectangle
    let boxed_rectange: Box<Rectangle> = Box::new(Rectangle { p1: origin(), p2: origin() });
    ...
    println!("Boxed rectangle occupies {} bytes in the stack", mem::size_of_val(&boxed_rectange));
