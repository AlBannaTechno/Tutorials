...
fn main() {
...

    {
        ...
        // This is very important, otherwise `ws` wouldn't start processing the input we just sent
    }

    // the `stdout` field also has type `Option<PipeStream>`
    // the `as_mut` method will return a mutable reference to the value wrapped in a `Some` variant
