...
fn main() {
    ...
    // Create a symbolic link, reaturns 'IoResult<()>'
    fs::symlink(&Path::new("../b.txt"), &Path::new("a/c/b.txt")).unwrap_or_else(|why| {
        println!("! {}", why.kind);
    });
    ...
    println!("'walk a'");
    // Recursively walk over the contents of a directory, returns 'Directories' which implements the
    // 'Iterator<Path>' trait
    ...
    fs::unlink(&Path::new("a/c/e.txt")).unwrap_or_else(|why| println!("! {}", why.kind));

    println!("'rmdir a/c/d'");
    // Remove an empty directory, returns 'IoResult<()>'
    fs::rmdir(&Path::new("a/c/d")).unwrap_or_else(|why| println!("! {}", why.kind));
}
