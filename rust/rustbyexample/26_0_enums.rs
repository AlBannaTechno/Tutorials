...
impl List {
    ...
    fn len(&self) -> uint {
        // 'self' has to be matched, because the behaviour of this method depends on the variant of
        // 'self'. 'self' has type '&List', and '*self' has type 'List', matching on a concrete type
        // 'T' is preferred over a match on a reference '&T'
        match *self {
