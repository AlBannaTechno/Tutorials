...
fn main() {
    ...
    println!("a containts: {}", a);

    // "Move" 'a' into 'b'
    // Here's what happens under the hood: the pointer 'a' gets copied (*not* the data on the heap,
    // just its address) into 'b'. Now both are pointers to the *same* heap allocated data. But now,
    // 'b' *owns* the heap allocated data; 'b' is now in charge of freeing the memory in the heap.
    let b = a;
    ...
    // "Move" 'b' into the function; 'b' gives up ownership of heap data
    destroy_box(b);
