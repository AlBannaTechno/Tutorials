fn main() {
    ...
    match reference {
        // If `reference`s is pattern matched again `&val`, it returns in a comparison like:
        // `&i32`
        // `&val`
        // ^ We see that if the matching `&`s are dropped, then the `i32` should be assigned to
        // `val`.
    ...
    // already a reference. This is not a reference because the right side is not.
