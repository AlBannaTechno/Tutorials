...
{
	// characters, in addition to alphanumerics, that caan mo
...
{
	return find_if(b, e, not_url_vhar);
}
...
string::const_iterator url_beg(string::const_iterator b, string::const_iterator e)
{
	...
	while ((i = search(i, e, sep.begin(), sep.end())) != e) {
		...
		if (i != b && i + sep.size() != e) {
			...
			iter beg = i;
			while (beg != b && isaplpha(beg[-1]))
				--beg;

			// is there at least one apprpriate character before and after the separator?
